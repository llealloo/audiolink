Shader "LTCGI/Surface_WorldSpaceTex"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            if(abs(worldNormal.x)>0.5)
            {
                ...
                c = tex2D(_MainTex, UV * _Scale + _Offset.yz); // use WALLSIDE texture
            }
            else if(abs(worldNormal.z)>0.5)
            {
                UV = IN.worldPos.xy; // front
                c = tex2D(_MainTex, UV * _Scale + _Offset.xy); // use WALL texture
            }
            else
            {
                UV = IN.worldPos.xz; // top
                c = tex2D(_MainTex, UV * _Scale + _Offset.xz); // use FLR texture
            }
            o.Albedo = c.rgb;
            o.Metallic = _Metallic;
            //o.Smoothness = tex2D (_GlossinessMap, IN.uv_MainTex);
            o.Smoothness = tex2D (_GlossinessMap, UV * _Scale);
            if (_MapIsRoughness)
